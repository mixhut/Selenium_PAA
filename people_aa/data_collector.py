...
from functions.foxwd_new import GSearch
...
def parse_args():
    ...
    parser.add_argument(
        "--input-file", "-i", help="input file which is a txt file containing list of questions",
        required=True)
    parser.add_argument(
        "--output-file", "-o", help="output file which is .json file containing a dictionnary of question: answer",
        required=True)
    parser.add_argument(
        "--proxy-file", "-p", help="proxy file containing list of proxy")
...
def collect_one_question(question):
    try:
        answer = GSearch.get_simple_answer(question)
